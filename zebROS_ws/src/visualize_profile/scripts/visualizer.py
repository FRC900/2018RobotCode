...
def create_plot(req):
	...
	for i in req.joint_trajectory.points:
		if(k%5 == 0):
			orient.arrow(scale*(-i.positions[1]+init_pos_y), scale*(i.positions[0]+init_pos_x), -arrow_scale*scale*math.sin(i.positions[2])*.5, arrow_scale*scale*math.cos(i.positions[2])*.5, head_width=scale*arrow_scale*0.1, head_length=scale*arrow_scale*0.25, fc='k', ec='k');
		k+=1

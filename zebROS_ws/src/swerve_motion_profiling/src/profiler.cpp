...
namespace swerve_profile
{
	...
	double max_steering_accel, double max_steering_vel, double dt, double index_dist_unit_)
	{
	...
	bool swerve_profiler::generate_profile(std::vector<path_point> path, double initial_v, double final_v)
	{
		...
		double v_curve_max; 
		double eff_max_a;
		double max_wheel_oriention_vel;
		double max_wheel_oriention_accel;
		//Forward pass
		positions.push_back(0);
		for(int i = 0; i < path.size();;)
		{

			i += curr_v*dt/index_dist_unit_;	
			velocities.push_back(curr_v);
			positions.push_back(i);
			max_wheel_orientation_accel = path[i].angular_accel * max_wheel_dist;
			...
			max_wheel_orientation_vel = path[i].angular_velocity * max_wheel_dist;
			...
			max_wheel_orientation_vel * sqrt(2)/2, 2) + pow(max_wheel_orientation_vel, 2) / 2) / max_wheel_vel);
	...
	bool swerve_profiler::solve_for_next_V(double &radius, double &current_v, double &angular_accel)
	{
		...
		2)/radius + sqrt(2) * angular_accel / 2), accel))
		{
			current_v += accel * dt_;
			return true;
		}
